...
int main()
{
	...
	turinys = pirmas.Menu();
			...
			while (turinys == 5) {
				if (nulinis > 0) {
					...
					if (checked == 0) {
						...
						checked++;
						datafile.close();
						sort(sarasas.begin(), sarasas.end(), [](const Studentas& lhs, const Studentas& rhs) {return lhs.egzaminas < rhs.egzaminas; });
					};
			...
			while (turinys == 10) {
				...
				partition_copy(sarasas.begin(), sarasas.end(), back_inserter(Geri), back_inserter(Blogi), [](Studentas& temper) {return temper.GetFinal() >= 5; });
